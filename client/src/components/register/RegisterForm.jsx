...
const RegisterForm = () => {
  ...
  const validate = () => {
    ...
    if (!PASSWORD_REGEX.test(password)) {
      toast.error(
        "Password must be Minimum 8 characters long and include 1 uppercase, 1 lowercase,1 number, and 1 special character.",
      );
      return false;
    }
    ...
    if (!agreeEula) {
      toast.error("Please accept the End User License Agreement.");
      return false;
    }
    if (!agreePrivacy) {
    ...
    if (country === "India") {
      if (!/^[6-9]\d{9}$/.test(mobile)) {
        toast.error(
          "Indian mobile number must be 10 digits starting with 6–9.",
        );
        return false;
      }
      if (!/^[1-9]\d{5}$/.test(postalCode)) {
        toast.error("Indian PIN code must be 6 digits.");
        return false;
      }
    }
    return true;
  ...
  const EmailStatus = () => {
    ...
    if (emailAvailable === false)
      return (
        <p className="mt-1 text-xs text-red-600">✗ Email already registered</p>
      );
    return null;
  };
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="Example: Doctor@123"
                      value={password}
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-green-600 p-1 flex items-center justify-center"
                    ...
                    <p className="mt-2 text-xs text-gray-500">
                      Minimum 8 characters and include 1 uppercase, 1
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full px-4 py-3 pr-12 border-2 rounded-lg focus:outline-none focus:ring-2 transition-all duration-200 text-sm sm:text-base ${
